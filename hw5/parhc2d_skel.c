...
void halo_exchange_2d_x(int rank, int rank_x, int rank_y, int size, int px, int py, int nx, int ny, int nxglob, int nyglob, double *x, double *y, double **T, double *xleftghost, double *xrightghost, double *sendbuf_x, double *recvbuf_x)
{
  ...
  // pack send buffer
  for(j=0; j<ny; j++){
    sendbuf_x[j] = T[0][j]; }// Left boundry
  // send and recv
  ...
  // pack send buffer
  for(j=0; j<ny; j++){
    sendbuf_x[j] = T[nx-1][j]; }//Right boundry
  // send and recv
...
void halo_exchange_2d_y(int rank, int rank_x, int rank_y, int size, int px, int py, int nx, int ny, int nxglob, int nyglob, double *x, double *y, double **T, double *ybotghost, double *ytopghost, double *sendbuf_y, double *recvbuf_y)
{
  ...
  for(i=0; i<nx; i++){
    sendbuf_y[i] = T[i][0];}
  // send and recv
  MPI_Sendrecv(sendbuf_y,nx,MPI_DOUBLE,bot_nb,0,recvbuf_y,nx,MPI_DOUBLE,top_nb,0,MPI_COMM_WORLD,&status);
  ...
  // send and recv
  MPI_Sendrecv(sendbuf_y,nx,MPI_DOUBLE,top_nb,1,recvbuf_y,nx,MPI_DOUBLE,bot_nb,1,MPI_COMM_WORLD,&status);
  // unpack recv buffer
  for(i=0 ; i<nx; i++){
...
int main(int argc, char** argv)
{
  ...
  if(rank!=0)
  {
     tst = sendarr_dbl[0]; ten = sendarr_dbl[1];     dt = sendarr_dbl[2];  t_print = sendarr_dbl[3];
  ...
  for(it=0; it<num_time_steps; it++)
  {
    ...
    printf("The time taken at it th step %d, time_taken at ith step = %lf\n",it,time_taken);
